...
exports.userForget = (req, res, next) => {
    ...
    User.findOne({ email: req.body.email }, (err, user) => {
        ...
        user.save()
        .then((user) => {
            forgetPassMail(user.email, `http://localhost:3000/forget/verify/U/${user.verifyToken}`)
            })
        .catch((err) => {
            return res.status(500).json({ err: err, success: false })
            })
...
exports.companyForget = (req, res, next) => {
    ...
    Company.findOne({ email: req.body.email }, (err, company) => {
        ...
        company.save()
        .then((company) => {
            forgetPassMail(company.email, `http://localhost:3000/forget/verify/C/${company.verifyToken}`)
        })
        .catch((err) => {
            return res.status(500).json({ err: err, success: false })
        })
